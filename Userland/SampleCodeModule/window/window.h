...
typedef HexColor (*ShapeFunction)(
    int, int, double,
    double); // First two are the x and y coordinates regarding the origin. The
             // second two are scale factors of the shape
 ...
 * To make a color in the result be 'transparent' in later operations, set its
 * HexColor opacity to 0. This function is for translating bitmap-equivalents to
 * HexColor arrays. Not for creating Windows. Chars set to 0 will be copied to
 * result as color 1, set to 1 copied as color 2, etc.
 *
 ...
 */
HexColor *toHexArray(char *source, HexColor *result, int width, int height,
...
/**
 * @brief Maps a portion of a char array onto HexColor array on a window, having
 * resized the char array to be the size of the screen
 *
 * @param w The Window
 ...
 * @param xOffset The x coordinate on the screen corresponding to the left edge
 * of the snippet to be mapped
 * @param yOffset The y cooridnate on the screen corresponding to the top edge
 * of the snippet to be mapped
...
Window overlayFromCharArray(Window w, char *source, uint64_t sourceWidth,
                            ...
                            uint64_t xOffset, uint64_t yOffset, OVERLAY_MODE m);
/**
 * @brief Maps a portion of a char array onto HexColor array on a Window,
 * resizing it to occupy the whole Window
 *
 * @param w The Window
 * @param source The char array
 * @param sourceWidth The true width of the char array
 ...
 *
 * @note Will default to drawWindow function if the screen is not the size of
 * the whole screen
 *
 * @param w
 * @return uint64_t
 ...
 *
 * @note Offsets refer to position of window top left corner regarding the
 * 'origin' of the ShapeFunction used.

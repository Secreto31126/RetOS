...
int main(int argc, char *argv[])
{
	if (argc < 0)
	{
		write(STD_OUT, "No matching string provided.", sizeof("No matching string provided") - 1);
		return 1;
	}
	...
	while ((n = read(STD_IN, buffer + remaining, BLOCK - 1 - remaining)) > 0)
	{
		...
		// if any parsing occurred, move what was not parsed to the start of the buffer
		if (fitFlag)
		{
		...
		// if the buffer did not have a single line divider, treat it as a line. The only alternative is using malloc, which would make this function unreadable and unwieldly (and a terrible waste of time)

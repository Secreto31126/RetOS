...
static const char prompt[] =
    "You are now in the module selector.\nPress 1 to continue to shell.\nPress "
...
void startModules() {
  ...
  setpriority(
      PRIO_PROC, getpid(),
      19); // Shell is usually very I/O bound, this prevents modules that are
           // not I/O bound from producing more input than the shell can print.
  ...
  while ((c = getChar()) != '5') {
    ...
    case '2': {
      ...
      char buffer[6];
      paintString(
          sPrintf("Time is %s.\n\nPress any key to return to module selector.",
    ...
    case '3': {
      ...
      if (!strcmp(c, ""))
        paintString("No dump has been generated. Press 'alt' to generate a "
                    "dump of the instant of pressing.\n\nPress any key to "
                    "return to module selector.",
                    -1, 0);
      else
        paintString(
            sPrintf("%s\n\nPress any key to return to module selector.", c), -1,
            0);
      freePrints();
      getChar();
      blank();
    ...
    case '4': {
      ...
      paintString("You are now in the exception tests module.\nPress 1 to test "
                  "the div-zero exception.\nPress 2 to test the invalid opcode "

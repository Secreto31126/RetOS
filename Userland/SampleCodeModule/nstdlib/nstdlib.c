...
uint64_t replaceWith(char *startAddress, char *replacement,
                     uint64_t eatThisManyChars);
...
char *getDumpString() {
  ...
  if ((lastAdded = get_dump(c + length - BIG_BLOCK, BIG_BLOCK))) {
    if (lastAdded >= length) {
      ...
      c = aux;
      length += BIG_BLOCK;
      while ((lastAdded = get_dump(c + length - BIG_BLOCK, BIG_BLOCK)) ==
             BIG_BLOCK) // adds BLOCK characters to the end of c until get_dump
                        // no longer has BLOCK characters to add
      {
  ...
  while (*format != 0 && *format != EOF) {
    if (*format == '%') {
      ...
      case 'l': {
        char aux[MAX_STDIN_STRING];
        char c;
        for (int auxReader = 0; auxReader < MAX_STDIN_STRING &&
                                (c = readChar()) >= '0' && c <= '9';
             auxReader++)
          aux[auxReader] = c;
        replaceWith(format - 1, aux, 2);
      ...
      default: {
        count--; // this is slightly inefficient, prevents adding count++ to
                 // every other condition in switch statement
        break;
      }
      }
      count++;
 ...
 * startAddress string should have enough space allocated to add insertion
 * string
 */
...
uint64_t replaceWith(char *startAddress, char *replacement,
  ...
  while (*aux != 0)
    aux++;    // aux is now looking at null termination of string containing
              // startAddress
...
// User is responsible for ensuring receiver has enough memory allocated to
// receive a char
...
uint64_t addString(char **receiver, uint64_t *length, char *source,
                   uint64_t *allocated) {
  uint64_t len = strlen(source);
  if ((*length + len) >= *allocated) {
    uint64_t toAdd =
        ((*length + len - *allocated) / BLOCK) * BLOCK +
        BLOCK; // space that must be added, rounded to nearest block
...
static char **allocatedPrints = null; // A pointer to pointers to char. Stores
                                      // all allocated strings in sPrintf.
static int allocatedPrintCount =
    0; // These serve to keep track of allocated memory in sPrintf and other
       // functions that allocate strings, so as not to offload free
       // responsibility to users.
void addToAllocated(char *address) {
  if (!(allocatedPrintCount % BLOCK)) {
    char **aux = realloc(allocatedPrints,
                         (allocatedPrintCount + BLOCK) * sizeof(char *));
    if (aux == NULL)
      return;
    allocatedPrints = aux;
  }
...
char *sPrintf(char *format, ...) {
  ...
  *toReturn = 0;
  while (*format != 0 && *format != EOF) {
    if (count == allocated) // Always checks if resize is necessary before
                            // checking to add any characters. Prevents copying
                            // code in every single-character edition.
    {
    ...
    if (*format == '%') {
      ...
      case 'd': {
        char aux[MAX_DIGITS_IN_LONG];
        addString(&toReturn, &count, itoa(va_arg(argp, int), aux, 10),
                  &allocated);
        break;
      }
      case 'u': {
        char aux[MAX_DIGITS_IN_LONG];
        addString(&toReturn, &count, utoa(va_arg(argp, int), aux, 10),
                  &allocated);
        break;
      }
      case 'l': {
        char aux[MAX_DIGITS_IN_LONG];
        addString(&toReturn, &count, ultoa(va_arg(argp, int), aux, 10),
                  &allocated);
        break;
      }
      default: {
...
char *getTimeString(char *buffer) {
  ...
  *buffer = 0;
  return buffer -
         5; // In total, buffer was shifted 5 spaces to format as 'HH:MM\0'
}
...
char *shiftToNextWord(char *s) { return shiftToWord(shiftToWhitespace(s)); }

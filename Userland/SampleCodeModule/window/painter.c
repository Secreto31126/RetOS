...
#define X_LINE_END                                                             \
  (((int)(width / (size * TRUE_LETTER_WIDTH))) * size * TRUE_LETTER_WIDTH)
...
void startPainter(uint64_t w, uint64_t h) {
  ...
  height = h;
  stamp = getWindow(
      TRUE_LETTER_WIDTH, TRUE_LETTER_HEIGHT,
      malloc(TRUE_LETTER_HEIGHT * TRUE_LETTER_WIDTH * sizeof(HexColor)));
...
void setSize(double s) {
  ...
  free(stamp.pixels);
  stamp.pixels = malloc(
      ((int)(TRUE_LETTER_HEIGHT * size)) * ((int)(TRUE_LETTER_WIDTH * size)) *
      sizeof(HexColor)); // realloc unnecessary, as all characters are only
                         // temporarily stamped
  if (stamp.pixels == NULL)
    exit(1);
}
uint64_t getSize() { return (uint64_t)(size + 0.5); }
void newLine() {
  yPointer += TRUE_LETTER_HEIGHT * size;
  xPointer = 0; // xPointer leaves room for header
}
void moveCursor() { xPointer += TRUE_LETTER_WIDTH * size; }

/**
 * @brief Moves the cursos back one character, paints a blank character, moves
 * the cursor back again character
 *
 */
void paintBackSpace() {
  if (xPointer <= 0) {
...
char paintChar(char c, HexColor letterColor, HexColor highlightColor) {
  if (c == '\b') {
    if (!(xPointer <= 0 &&
          yPointer <= 0)) // shouldn't be able to backspace through line
                          // starters, but shell should handle that.
    {
      paintBackSpace();
    }
    return 1;
  ...
  if ((xPointer + TRUE_LETTER_WIDTH * size) > width || c == '\n') {
    if ((yPointer + TRUE_LETTER_HEIGHT * size * 2) >
        height) // *2 because letters are drawn from cursor downwards and to the
                // right, so otherwise last line would have its top on the
...
uint64_t maxYPointer() { return height - 2 * size * TRUE_LETTER_HEIGHT; }

uint64_t maxXPointer() { return width - TRUE_LETTER_WIDTH * size; }

char willFit(const char *s) {
  double xP = 0, yP = 0, maxX = width - TRUE_LETTER_WIDTH * size,
         maxY = height - 2 * TRUE_LETTER_HEIGHT * size;
  while (*s) {
    if (*s == '\n' || xP > maxX) {
      xP = 0;
...
void endPainter() { freeWindow(stamp); }
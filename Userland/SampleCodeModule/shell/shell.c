...
static HexColor letterColor = 0xFF000000 | HEX_WHITE,
                highlightColor = 0xFF000000 | HEX_BLACK;
...
static commandData activeReads[MAX_ACTIVE] = {
    {{NULL, STD_IN, -1, -1}, NULL, 0}};
 ...
 *
 * @param index The index of the module to be removed in the list of modules
 * monitored by the shell
 */
...
/**
 * @brief Get the index of a read fd in the list of modules monitored by the
 * shell
 *
...
/**
 * @brief Bring a module from the list of modules monitored by the shell to
 * foreground
 *
 ...
 * @return 0 if no additional actino must be taken, 1 if the user requested to
 * exit the shell, 2 if the user requested to bring a module to foreground
 */
  ...
  if (c == '\b') {
    ...
    if (c == '\n') {
      ...
      addStringToBuffer(
          ...
          1); // Nota: las instrucciones del TPE especifican que la shell debe
              // moverse hacia arriba si se excede su espacio para texto.
      freePrints(); // No se especifica el comportamiento esperado si el retorno
                    // de un 'comando' es mayor al espacio de la shell.
      commandIndex = 0; // El comportamiento default es simplemente no imprimir
                        // el retorno completo. Esto es lo que ocurre si a
                        // passCommand se le da parámetro 'ask' 0.
    } // De lo contrario, se imprimirá moviendo hacia arriba de a MOVE_BY
      // líneas, requiriendo input del usuario.
    else {
      addCharToBuffer(c);
      fromLastEnter++;
      if (commandIndex < MAX_COMMAND_LENGTH)
        commandBuffer[commandIndex++] =
            c; // ignores commands after a certain length (since no commands
               // should be that long)
    }
  }
...
/**
 * @brief Reads from a module in the list of modules monitored by the shell as
 * if it were a background module
 *
 * @param index The index of the module in the list of modules monitored by the
 * shell
...
int readAsBackground(int index, displayStyles displayStyle) {
  ...
  if (aux == 2) {
    // in case the process did not end on its own, kill it, as it has no useful
    // output to give
    killModule(cData, "");
    removeFromActive(index);
    return 0;
...
char shellLoop() {
  ...
  for (int i = 0; i < n; i++) {
    if (canBeRead[i] == STD_IN) {
      ...
      // Handle all user input before printing bg processes (makes terminal more
      // reseponsive)
      return 0;
    } else {
 ...
 * @param ask Whether the user should be prompted before warping the screen if
 * the buffer does not fit
 */
void addStringToBuffer(const char *s, char ask) {
  const char *aux = s;
  while (*aux)
...
void paintStringOrWarp(const char *s, char ask) {
  ...
  char flag = 0;
  if (ask) {
    char *prompt = "The output of this command will not fit. Press any key to "
                   "continue output.";
...
/**
 * @brief Interprets input from the STD_KEYS file descriptor, directs it to
 * module when relevant
 *
 * @param data The module currently in foreground
 * @param displayStyle How the module expects to be displayed
 * @return 0 if no action needs to be taken, 1 if communication with the module
 * failed or STD_KEYS, 2 if input requests the module be killed, 3 if input
 * requests the module be sent to background
 */
...
/**
 * @brief Interprets output from a module's read file descriptor, reproduces it
 * in the CLI
 *
 * @param data The module to interpret
 * @param displayStyle How the module expects to be displayed
 * @return 0 if no action needs to be taken, 1 if communication with the module
 * failed, 2 if the module is dead
 */
int handleReadFd(moduleData data, displayStyles displayStyle) {
...
/**
 * @brief Interprets input from the STD_IN file descriptor, directs it to the
 * module when relevant, reproduces it on the CLI when relevant
 *
 * @param data The module currently in foreground
 * @param displayStyle How the module expects to be displayed
 * @return 0 if no action needs to be taken, 1 if communication with the module
...
/**
 * @brief Sends SIGKILL to all process ids associated with a module, waits on
 * all these process ids, closes all open file descriptors associated with the
 * module
 *
...
/**
 * @brief Read from a module as foreground until it is closed or otherwise
 * stopped
 *
...
void readUntilClose(commandData cData, displayStyles displayStyle) {
  ...
  int fullReadFds[MAX_ACTIVE + 2] = {data.fd, STD_IN, STD_KEYS},
                               fullReadFdCount,
                               fullAvailableReadFds[MAX_ACTIVE + 2] = {0},
                               fullAvailableReadFdCount;
  ...
  while (!leaveFlag) {
    ...
    // Do not start from activeReads[0], as STD_IN is already here. It is best
    // if STD_IN input is processed first (looks better on terminal)
    for (int i = 3; i < fullReadFdCount; i++) {
      fullReadFds[i] = activeReads[i - 2].data.fd;
    }
    fullAvailableReadFdCount =
        pselect(fullReadFdCount, fullReadFds, fullAvailableReadFds);
    for (int i = 0; i < fullAvailableReadFdCount; i++) {
      ...
      if (data.fd == currentFd) {
        ...
        if (aux == 1) {
          killModule(cData,
                     "Communication failed, killed foreground process.\n");
          return;
        }
        if (aux ==
            2) { // process stopped writing, wait for it to terminate normally
          leaveFlag = 1;
          break;
        }
      }
      // a key has been pressed
        ...
        if (aux == 2) { // send SIGKILL to process, wait for it to terminate
          killModule(cData, "");
          return;
        }
        if (aux == 3) { // user wants it moved to bg
...
char *passCommand(char *toPass) {
  ...
  if (displayStyle >= REDRAW_ONCE) {
    ...
    index = 0;
    buffer[index] = 0;
    if (willFit(toReturn)) // so as not to blank when it won't fit, as the
                           // painting functions will blank anyways
      blank();
    return toReturn;
 ...
 */
void setLetterColor(HexColor color) // command handler is responsible for
                                    // setting displayStyle to REDRAW_ONCE
{
 ...
 */
void setHighlightColor(HexColor color) // command handler is responsible for
                                       // setting displayStyle to REDRAW_ONCE
{
 ...
 */
void resize(double size) // command handler is responsible for setting
                         // displayStyle to REDRAW_ONCE
{
  setSize(size);
}
/**
 ...
 * @param n
 */
void warpNLines(uint64_t n) // char or char* you want to add must be in buffer
                            // already. This shortens the buffer from the start
                            // so that it fits, then repaints it.
{
  if (!n)
    return;

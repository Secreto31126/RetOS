...
static int exists(const char *name, int *i) {
  ...
  for (size_t j = 0; j < MAX_SEMS; j++) {
    ...
    if (semaphores[j].sem) {
      if (*semaphores[j].sem->name &&
          strncmp(semaphores[j].sem->name, name,
                  sizeof(semaphores[j].sem->name)) == 0) {
        *i = j;
        return 1;
      }
    } else if (free == -1) {
...
static int usable(const sem_t *sem) { return sem && sem->value != INVALID_SEM; }

static bool semaphore_signal() { return false; }
...
static bool semaphore_unlocked(pid_t pid) { return true; }

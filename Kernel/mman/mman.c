...
size_t mem_size(size_t index) { return (MEM_END - MEM_START) >> height(index); }
...
size_t find_buddy(size_t size, size_t index, size_t current_size) {
  ...
  // that can fit it If current_size is smaller than BLOCK, you have reached
  ...
  if (read_state(MAP_START, index) == FULL ||
      read_state(MAP_START, index) == ALLOCATED) {
    return -1;
  }
...
char *print_rec(char *output, char *end, size_t i, size_t height,
                display_empties empties);

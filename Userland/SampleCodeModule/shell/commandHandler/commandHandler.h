 ...
 * @param cPidBuffer Return variable, pids of all processes created for the
 * module (does not include processes the module itself may create)
 * @return Data for the module created, either a string or an fd to read from
 * (NULL or -1 when not created), an fd to write to the module (-1 if not
 * created), an array of pids created for the module
 */
commandData handleCommand(char *command, displayStyles *displayStyle,
                          int *cPidBuffer); // All it does.
/**

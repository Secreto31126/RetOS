...
unsigned int sleep(unsigned int seconds) {
  Process *p = get_current_process();
  add_tick_blocked(p, sleep_finished, NULL + (get_tick() + seconds * 18), NULL,
                   NULL, NULL, NULL);
  sched_yield();
...
unsigned int usleep(unsigned int usec) {
  Process *p = get_current_process();
  add_tick_blocked(p, sleep_finished, NULL + (get_tick() + usec), NULL, NULL,
                   NULL, NULL);
  sched_yield();
...
int pselect(int nfds, const int *fds, int *ready) {
  ...
  add_tick_blocked(p, multi_read_available, NULL + nfds, copy_fds, ready_ptr,
                   NULL, NULL);
  sched_yield();

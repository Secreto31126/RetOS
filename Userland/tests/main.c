...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(test, "endless_loop")) {
    my_nice(my_getpid(), 19); // Since I'm not too eager to give extra cpu time
                              // to a process stuck in a busy wait
    endless_loop();
    return 0;
  }
  ...
  if (!strcmp(test, "testmm")) {
    ...
    puts("Test done.");
    if (aux == -2)
      puts("\nTest reported incorrect parameters. This test expects one "
  ...
  if (!strcmp(test, "testsync")) {
    ...
    puts("Test done.");
    if (aux == -2)
      puts("\nTest reported incorrect parameters. This test expects two "
           "parameters: inc, use_sem");
    else if (aux < 0)
      puts("\nTest reported an error.");
    return aux;
  }

...
Process *get_current_process() { return get_process(pid); }

Process *get_process(pid_t p) { return processes + p; }
...
FIND_PID:
  ...
  while (new_pid < MAX_PROCESSES &&
         processes[new_pid].state != NOT_THE_PROCESS_YOU_ARE_LOOKING_FOR) {
    new_pid++;
  }
  ...
  size_t new_stack_size =
      parent->stack_size > MIN_STACK_SIZE ? parent->stack_size : MIN_STACK_SIZE;
  ...
  memcpy(STACK_END(new_stack, new_stack_size) - parent->running_stack_size,
         parent->running_stack, parent->running_stack_size);
...
void set_exit_code(int status) { get_current_process()->exit_code = status; }
...
int kill_process(pid_t pid) {
  ...
  if (man_im_dead->state == PROCESS_RUNNING &&
      man_im_dead->running_stack != man_im_dead->stack) {
    memcpy(man_im_dead->running_stack,
           STACK_END(man_im_dead->stack, man_im_dead->stack_size) -
  ...
  /**
   * @brief Wether the stack should be freed or not (another process is using it
   * as running stack)
   */
  bool free_stack = true;
  /**
   * @brief Wether another process moved into the stack (free_stack is set to
   * false)
   */
  bool stack_inherited = false;
  /**
   * @brief Wether the running stack should be freed or not (another process is
   * using it as running stack)
   */
  ...
  while (p) {
    ...
    if (p->running_stack == man_im_dead->stack) {
      // If a process still needs the stack to run (pseudo pagination), don't
      // free it
      free_stack = false;
    ...
    if (p->running_stack == man_im_dead->running_stack) {
      // If a process still needs the running stack to run (pseudo pagination),
      // don't free it
  ...
  for (size_t i = 0;
       i < MAX_PROCESS_FILES && count && (free_stack || free_running_stack);
       i++) {
    Process *p = get_process(i);
...
int dup2(int oldfd, int newfd) {
  ...
  if (0 <= oldfd && oldfd < MAX_PROCESS_FILES && 0 <= newfd &&
      newfd < MAX_PROCESS_FILES) {
    close(newfd);

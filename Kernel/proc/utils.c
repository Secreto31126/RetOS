...
bool inherit_parents_house(Process *process) {
  ...
  memcpy(STACK_END(process->running_stack, process->running_stack_size) -
             process->stack_size,
         process->stack, process->stack_size);

  // The stack can be freed without checking other process because:
  // - If the process didn't have a child, the stack isn't being used by any
  // other process
  // - If the process had a child before execv, it will be using the original
  // running stack, not the parent's stack
  // - If the process had a child after execv, the process wouldn't have
  // different stack and running stack
  free(process->stack);

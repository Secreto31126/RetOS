...
#define FRAMEBUFFER                                                            \
  ((framebuffer_element *)((uint64_t)VBE_mode_info->framebuffer))
...
int putPixel(HexColor hexColor, uint64_t x, uint64_t y) {
  return putPixelStd(GET_OPACITY(hexColor), GET_RED(hexColor),
                     GET_GREEN(hexColor), GET_BLUE(hexColor), x, y);
}

int putPixelStd(uint8_t opacity, uint8_t red, uint8_t green, uint8_t blue,
...
static uint8_t mergeColor(uint8_t background, uint8_t overlay,
                          uint8_t opacity) {
  if (!opacity)
    return background;
...
uint64_t super_fast_fill_screen_shape(ShapeFunction f) {
  ...
  while (i < size) {
    ...
    putPixelStd(-1, GET_RED(color), GET_GREEN(color), GET_BLUE(color),
                i % width, i / width);
    i++;
  }
...
uint64_t drawShape(ShapeFunction f, uint32_t width, uint32_t height, uint32_t x,
                   uint32_t y) {
  if (!x && !y && width > VBE_mode_info->width &&
...
uint64_t drawScaledShape(ShapeFunction f, uint32_t width, uint32_t height,
                         uint32_t x, uint32_t y, double xScaleFactor,
                         double yScaleFactor) {
...
uint64_t drawFromArray(HexColor *array, uint32_t width, uint32_t height,
...
uint16_t get_width() { return VBE_mode_info->width; }

uint16_t get_height() { return VBE_mode_info->height; }

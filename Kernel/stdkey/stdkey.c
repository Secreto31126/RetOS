...
int init_stdkey() { return sem_init(&sem, 1, 0); }
...
void stdkey_keyevent(uint8_t scancode) {
  // If the scancode is a release event, release the key from the pressed keys
  // array
...
bool stdkey_empty() { return reader == writer && !on_your_left; }

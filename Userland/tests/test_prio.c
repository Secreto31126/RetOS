...
#define MINOR_WAIT                                                             \
  "5000000" // TODO: Change this value to prevent a process from flooding the
            // screen
#define WAIT                                                                   \
  300000000 // TODO: Change this value to make the wait long enough to see these
            // processes beeing run at least twice

#define TOTAL_PROCESSES 3
#define LOWEST                                                                 \
  -10 // Note: Not the actual minimum priority. That is -19, with which the
      // process would run too infrequently
...
void test_prio(uint64_t argc, char *argv[]) {
  ...
  int pids[TOTAL_PROCESSES]; // MODIFIED TO ACCEPT NEGATIVE PIDS (I.E.,
                             // my_create_process FAILED)
  uint64_t i;
